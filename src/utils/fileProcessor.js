...
export async function process3DFile(file) {
    ...
    if (fileName.endsWith('.obj') || fileName.endsWith('.glb') || fileName.endsWith('.gltf') || fileName.endsWith('.fbx') || fileName.endsWith('.dae')) {
        return {
    ...
    if (fileName.endsWith('.zip')) {
        ...
        zip.forEach((relativePath, zipEntry) => {
            ...
            const promise = zipEntry.async('blob').then(blob => {
                ...
                // Buscar archivo principal (prioridad a GLB/GLTF, luego OBJ/FBX/DAE)
                ...
                } else if (lowerName.endsWith('.dae') && !mainFile) {
                    mainFile = { name: entryName, url: blobUrl, type: 'dae' };
                }
            });
        ...
        if (!mainFile) {
            throw new Error("El archivo ZIP no contiene ningún modelo 3D válido (.obj, .glb, .gltf, .fbx, .dae)");
        }
    ...
    throw new Error("Formato de archivo no soportado. Use .obj, .glb, .gltf, .fbx, .dae o .zip");
}
